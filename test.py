...
from eval_spectra_pose import evaluator
...
def main(args, model_cls=ContourPose, batch_size=8, threshold=5,
    ...
    # Reproducibility hook: pin every RNG the eval path touches.
    # Does NOT alter the algorithm, metric, or PECP hyperparameters — the
    # authors' code left random.choice (PECP subset sampling) and OpenCV's
    # solvePnPRansac unseeded, so seeded and unseeded runs sample the same
    # distribution. Pinning makes each individual run reproducible, which
    # the variance-characterization harness relies on.
    ...
    if eval_seed is not None:
        ...
        print(f"[Test] eval_seed={eval_seed} (random / numpy / cv2 seeded)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_seed", type=int, default=None,
                        help="Seed random / numpy / cv2 RNGs before eval. "
                             "Reproducibility only — does not change the algorithm. "
                             "Leave unset for legacy unseeded behavior.")
    parser.add_argument("--use_masks", type=str, default="false",
                        choices=["true", "false", "both"],
                        help="Use masked RGB: true, false, or both (default: false). "
                             "false matches the legacy eval which reads photo_cut as-is; "
                             "true zeroes out non-object pixels before inference and "
                             "does NOT match the training-time input distribution.")
